...
static t_vector	set_axis_rotate_base(void)
{
	return ((t_vector){\
...
t_screen_info	get_screen_info(t_scene *scene)
{
	...
	screen.rotate_x = get_rotate_x(scene->camera->dir_norm, base);
	screen.rotate_y = get_rotate_y(scene->camera->dir_norm, base, rotate_base);
	return (screen);
}
